...
const {arraysToArray} = require("./functions")
...
router.get("/recipes", async (req,res) =>{
  try{
    ...
    let limit = 20;
    let first100ApiRecipes = await axios.get( `https://api.spoonacular.com/recipes/complexSearch?&addRecipeInformation=true&information?includeNutrition=false&number=${limit}&apiKey=${APIKEY4}`
    ) .then((d) => {
      delete d.data.extendedIngredients;
      return d.data.results;
    }).catch(r => r);

    let allNewRecipes = await Recipes.findAll({
        include: [
            { model: Diets,
              attributes: ['list']
            },
...
// [ ] Posibilidad de seleccionar/agregar uno o más tipos de dietas
    ...
    router.post("/recipe", async (req, res) => {   
      ...
      //       let result = await axios.get(`https://api.spoonacular.com/recipes/complexSearch?query=${name}&addRecipeInformation=true&apiKey=${APIKEY3}`)
